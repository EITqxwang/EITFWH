...
// Define the time values for observers；存储在obstime中
void Farrasat1A::InitializeObserverTime(double tMin, double tMax, size_t tNum)
{
	obsTimeMin = tMin;
	obsTimeMax = tMax;
	obsTimeNum = tNum;
	double dt = (obsTimeMax - obsTimeMin) / (obsTimeNum - 1.0);//时间间隔
...
void Farrasat1A::CalTimeDelayAtStep(size_t it) {
	...
	timeDelay.resize(srcNum); //400,
	R.resize(srcNum);
	Rhat.resize(srcNum);
	for (size_t i = 0; i < srcNum; i++)
	{
		timeDelay[i].resize(obsLocation.size()); //400,30
		R[i].resize(obsLocation.size());
		Rhat[i].resize(obsLocation.size());
	}

	const vec3 Mo = { -M0, 0., 0. };//"FreeStreamMachNumber"
	const double betaSqr = 1.0 - M0 * M0;

	for (size_t i = 0; i < srcNum; i++) //400
	{
		for (size_t j = 0; j < obsLocation.size(); j++) //30
		{
...
void Farrasat1A::CalTimeDelayMinMax() {
	...
	CalTimeDelayAtStep((size_t)0); // compute time delay at the first step
...
void Farrasat1A::CalSourceTerms() {
	...
	srcQn.resize(srcTime.size());//5000
	srcLi.resize(srcTime.size());
	for (size_t i = 0; i < srcTime.size(); i++)
	{
		srcQn[i].resize(srcNum);//400，面单元数
	...
	for (size_t i = 0; i < srcTime.size(); i++)
	{
		...
		for (size_t j = 0; j < srcNum; j++)//面单元数量
		{
			const double& rho = srcDensity[i][j];
			const vec3& n = srcNormVec[i][j];//积分面单位法向
			...
			srcLi[i][j] = p * n + rho * u * dot(u - v, n);//源项Li，忽略τij
		}
	}
...
{
	srcQn2D.resize(srcTime2D.size());//5000
	srcLi2D.resize(srcTime2D.size());
	
	size_t srcNum2D = srcLocation2D[0].size();//360
	
	for (size_t i = 0; i < srcTime2D.size(); i++)
	{
		srcQn2D[i].resize(srcNum2D);//360，面单元数
		srcLi2D[i].resize(srcNum2D);
	}

	for (size_t i = 0; i < srcTime2D.size(); i++)//5000
	{
		if (isMaster)   cout << "\r";
		if (isMaster)  fcout << "Progress: " << floor(100 * (i + 1) / srcTime2D.size()) << " % " << flush;
		for (size_t j = 0; j < srcNum2D; j++)//面单元数量360
		{
			
			const double& rho = srcDensity2D[i][j];//总密度
			const vec3& n = srcNormVec2D[i][j];//单元面法线方向
			const vec3& u = srcVelocity2D[i][j];//注意速度是否包含环境速度；
			double p = srcPressure2D[i][j];   //总压力
			
			// relative to a uniform x0 medium
			vec3 U0= vec3(C0 * M0, 0.0, 0.0);
			
			// compute source terms
			vec3 Qi = rho * u - rho0 * U0;//源项Q计算
			srcQn2D[i][j] = dot(Qi, n);

			// We have neglected the viscous shear force over the data surface 
			
			srcLi2D[i][j] = (p-p0) * n + rho * (u - 2 * U0) * dot(u, n)+ rho0 * U0 * dot(U0, n);//源项Li
...
void Farrasat1A::InterpTimeDerivative() {
	...
	const double deltaT = obsTime[1] - obsTime[0];
	srcQnDot.resize(srcTime.size());//5000
	srcLiDot.resize(srcTime.size());
	for (size_t i = 0; i < srcTime.size(); i++)
	{
		srcQnDot[i].resize(srcNum);//面单元数量
...
void Farrasat1A::CalTimeSignal() {
	...
	pPrime.resize(obsTimeNum, vector<double>(obsLocation.size(), 0.0));//5000，30
	...
	vec3 M(-M0, 0, 0);
	double MSqr = M.magSqr();//intenstion: sqrt()

	for (size_t i = 0; i < srcTime.size(); i++)//5000
	{
		if (isMaster)   cout << "\r";
		if (isMaster)  fcout << "Progress: " << floor(100 * (i + 1) / srcTime.size()) << " % " << flush;
		...
		for (size_t j = 0; j < srcNum; j++)//面单元数量
		{
			...
			const vec3& LiDot = srcLiDot[i][j];//关于源的时间导数项
			double LM = dot(M, Li);//

			for (size_t k = 0; k < obsLocation.size(); k++)//60
			{
				const double& rMag = R[j][k];	// TODO: may be better to use [j][k]
				const vec3& rHat = Rhat[j][k];//指向观察者的单位矢量
...
{
	CalSourceTerms2D();
	size_t srcTNum = srcTime2D.size();//5000
	size_t srcFrequencyNum = (size_t)floor(srcTNum / 2) + 1;//2501
	//在频率域中，由于对称性，通常只需要一半的数据
	size_t srcNum2D = srcLocation2D[0].size();//360
	double* in_real;
	fftw_complex* dft_cplx;
	// allocate memory 输入和输出
	in_real = (double*)fftw_malloc(sizeof(double) * srcTNum);//动态分配double类型的数组，数组大小为srcTNum
	...
	double scaleFactor = 2.0 / srcTNum;//2/5000
	if (isMaster)  fcout << "Calculate 2D Fourier transform " << endl;
	for (int ip = 0; ip < srcNum2D; ip++) // loop all source pannel 360
	{
		if (isMaster)   cout << "\r";
		...
		for (int ik = 0; ik < srcFrequencyNum; ik++)//2501，数据对称
		{
	...
	srcFrequency2D.resize(srcFrequencyNum, 0.0);//2501
	double deltaF = samplingFrequency / srcTNum;//5000
	for (int i = 0; i < srcFrequencyNum; i++) {
		srcFrequency2D[i] = double(i) * deltaF;
	}//生成频率轴
	srcFrequency2D[0] = 1E-8;//*****************初始位置频率偏移
	...
	for (int ik = 0; ik < srcFrequencyNum; ik++)
	{
		...
		for (int ip = 0; ip < srcNum2D; ip++)//360
		{
			...
			for (int io = 0; io < obsLocation.size(); io++)//60
			{
				...
				//
				pPrimeCplx2D[ik][io] += pTotal;//2501,60
			}
		}
	}
	if (isMaster)   cout << endl;
...
complex<double> Farrasat1A::Green(double xpos1, double xpos2, double ypos1, double ypos2,double omega1)
{
	...
	double H2 = cyl_neumann(0, Hk);//x=0时趋于无穷
	complex<double> Hkel = H1 - I * H2;
	//格林函数计算
	complex<double> Gn = I / 4.0 / sqrt(betasqr) * exp(I * M * omega1 / C0 / betasqr * r1) * Hkel;//注意M的正负
	return Gn;
...
void Farrasat1A::CalFreqSpectra() {
	...
	size_t srcTNum = srcTime.size();//5000
	size_t srcFrequencyNum = (size_t) floor(srcTNum / 2) + 1;//2501
	// 在频率域中，由于对称性，通常只需要一半的数据
	// declare real number arrays for 1d-fft real input / complex output
	double* in_real;
	fftw_complex* dft_cplx;
	// allocate memory 输入and输出
	in_real = (double*)fftw_malloc(sizeof(double) * srcTNum);//动态分配double类型的数组，数组大小为srcTNum
	dft_cplx = (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * srcFrequencyNum);
	...
	// allocate memory space for pressure at observers 2501,400
	...
	for (int ip = 0; ip < srcNum; ip++) // loop all source pannel 400
	{
		if (isMaster)   cout << "\r";
	...
	const double samplingFrequency = 1.0 / deltaT;
	srcFrequency.resize(srcFrequencyNum, 0.0);//2501
	double deltaF = samplingFrequency / srcTNum;//5000
	for (int i = 0; i < srcFrequencyNum; i++) {
	...
	CalTimeDelayAtStep(0);//引入半径r，r矢量
	//2501；60
	...
	for (int ik = 0; ik < srcFrequencyNum; ik++)
	{
		...
		for (int ip = 0; ip < srcNum; ip++)//400
		{
			...
			for (int io = 0; io < obsLocation.size(); io++)//60
			{
				...
				// 
				pPrimeCplx[ik][io] += pTotal * exp(-I * omega * td);//************+-I
			}
		}
	}
	if (isMaster)   cout << endl;
...
void Farrasat1A::RecoverSignals()
{
	...
	pPrime.resize(obsTimeNum, vector<double>(obsLocation.size(), 0.0));//5000;60
	for (int io = 0; io < obsLocation.size(); io++)
	{
		if (isMaster)   cout << "\r";
		if (isMaster)  fcout << "Progress: " << floor(100 * (io + 1) / obsLocation.size()) << " % " << flush;
		...
		fftw_execute(ift_plan);
		for (int it = 0; it < obsTime.size(); it++)//5000
		{
...
void Farrasat1A::SaveSpectrumMag2D(string pFile) {
	...
	ofstream spectrumFile_mag(pFile.c_str());//srcFrequency2D
